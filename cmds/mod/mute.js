module.exports.run = async (bot, msg, args) => {
  ...
  if (!msg.member.hasPermission("ADMINISTRATOR")) {
    msg.channel.send("You don't have administrator perms.").then((msg) => {
  ...
  if (!user) {
    msg.channel.send("Please specify a user via mention or ID.").then((msg) => {
      msg.delete({ timeout: 5000 });
    });
    return;
  }
  ...
  if (reason === " ") {
    msg.channel.send("Please specify a reason.").then((msg) => {
      msg.delete({ timeout: 5000 });
    });
    return;
  }
  ...
  if (msg.author.id === user.id) {
    msg.channel.send("You can't warn yourself dumb, dumb.").then((msg) => {
      msg.delete({ timeout: 5000 });
    });
    return;
  }
  ...
  if (msg.guild.owner.id === user.id) {
    msg.channel.send("You can't unmute the server's owner.").then((msg) => {
      msg.delete({ timeout: 5000 });
    });
    return;
  }
  ...
  if (muted === null) {
    ...
    mentioned
      .send(`You have been muted in **${msg.guild.name}** for ${reason}`)
